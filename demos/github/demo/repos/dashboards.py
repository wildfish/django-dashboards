...
def get_change_stat_data(prop, **kwargs):
    ...
    obj = (
        Repo.objects.filter(full_name=full_name.lower().strip()).first()
        if full_name
        else None
    )

    if not obj or obj.latest_stats is None:
        return {"text": "-", "sub_text": "-", "full_name": full_name}
...
class RepoDashboard(Dashboard):
    form = Form(
        form=SearchForm,
        dependents=["stars", "watchers", "open_issues", "forks"],
        method="post",
    )
    stars = SSEStat(
        value=lambda *args, **kwargs: get_change_stat_data(
            "stars_count", *args, **kwargs
        )
    )
    watchers = SSEStat(
        value=lambda *args, **kwargs: get_change_stat_data(
            "watchers_count", *args, **kwargs
        )
    )
    open_issues = SSEStat(
        value=lambda *args, **kwargs: get_change_stat_data(
            "open_issues_count", *args, **kwargs
        )
    )
    forks = SSEStat(
        value=lambda *args, **kwargs: get_change_stat_data(
            "forks_count", *args, **kwargs
        )
    )
