...
Initial work on the Wildcoeus idea of building a toolkit/possible open source project to support Django dash like dashboards. 
...
- `/django-wildcoeus/wildcoeus` is the library, which will eventually become this os project, the tool we install in clients projects.
  ...
  - More specific notes/todos in `/django-wildcoeus/README.md`
- `/demo`is a Django MPA using `wildcoeus` to mock up how we'd use the lib 
  - Note that for ease the graphql endpoint for the SPA is also part of this Django project.
- `/demo-spa` is a Next.js version of demo, using `wildcoeus` and graphql to fetch the structure. 
