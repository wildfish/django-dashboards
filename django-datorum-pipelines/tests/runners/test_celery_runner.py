...
from datorum_pipelines import BasePipeline, BaseTask, PipelineTaskStatus
from datorum_pipelines.runners.celery.runner import Runner
...
def validate_run_task(task, task_id, pipeline_id, input_data):
    assert task.name == "datorum_pipelines.runners.celery.tasks.run_task"
...
def test_task_have_no_parents___tasks_are_added_to_chain_in_configured_order():
    ...
    validate_pipeline_report(
        task=chain.tasks[0],
        name="datorum_pipelines.runners.celery.tasks.run_pipeline_report",
...
def test_task__link_error_added():
    ...
    assert (
        ...
        == "datorum_pipelines.runners.celery.tasks.run_pipeline_report"
    )
